...
async function seedData() {
  ...
  try {
    ...
    const communities = await sql`
      ...
      VALUES 
        ('Sunset Manor', 'sunset-manor', 'Luxury senior living with breathtaking sunset views', 'Experience the finest in senior living at Sunset Manor, where every day ends with spectacular views and caring service.', '123 Sunset Blvd, Paradise Valley, AZ 85253', '(480) 555-0123', 'independent-living', '{"fitness-center", "dining-room", "pool", "library"}', 3500, true, true),
        ('Garden Oaks Community', 'garden-oaks', 'Peaceful assisted living surrounded by beautiful gardens', 'Garden Oaks offers compassionate assisted living care in a serene, garden-filled environment.', '456 Oak Garden Way, Scottsdale, AZ 85260', '(480) 555-0456', 'assisted-living', '{"gardens", "dining-room", "activities-room", "wellness-center"}', 4200, true, false),
        ('Heritage Hills', 'heritage-hills', 'Memory care with dignity and compassion', 'Specialized memory care services in a warm, homelike environment designed for those with Alzheimer\'s and dementia.', '789 Heritage Dr, Tempe, AZ 85284', '(480) 555-0789', 'memory-care', '{"secure-environment", "therapeutic-gardens", "specialized-care"}', 5800, true, true)
      RETURNING id, name
    `;
    ...
    const posts = await sql`
      ...
      VALUES 
        ...
        ('Garden Oaks: A Peaceful Haven', 'garden-oaks-peaceful-haven', 'At Garden Oaks Community, residents enjoy the tranquility of beautifully landscaped gardens while receiving the assisted living care they need. Our compassionate staff ensures every resident feels at home.', 'Discover the serene environment at Garden Oaks.', '{"community", "gardens", "assisted-living"}', $2, NOW()),
      ...
      RETURNING id, title
    ` + [sunsetManor.id, gardenOaks.id, heritageHills.id];
    ...
    const events = await sql`
      ...
      VALUES 
        ('Holiday Concert Series', 'holiday-concert-series', 'Join us for a festive evening of holiday music performed by local musicians. Light refreshments will be served.', NOW() + INTERVAL '7 days', NOW() + INTERVAL '7 days' + INTERVAL '2 hours', 'Main Auditorium', $1, 50),
        ('Gardening Workshop', 'gardening-workshop', 'Learn about container gardening and seasonal plants with our horticulture expert. Perfect for all skill levels.', NOW() + INTERVAL '10 days', NOW() + INTERVAL '10 days' + INTERVAL '1.5 hours', 'Garden Pavilion', $2, 20),
        ('Family Support Group', 'family-support-group', 'Monthly support group for families of residents with memory care needs. Facilitated by licensed social workers.', NOW() + INTERVAL '14 days', NOW() + INTERVAL '14 days' + INTERVAL '1 hour', 'Conference Room', $3, 15)
      RETURNING id, title
    ` + [sunsetManor.id, gardenOaks.id, heritageHills.id];
    ...
    const faqs = await sql`
      ...
      VALUES 
        ('What is included in the monthly fee?', 'Our monthly fee includes utilities, housekeeping, maintenance, dining services, and access to all community amenities.', 'pricing', $1, 1),
        ('Do you allow pets?', 'Yes, we welcome small pets under 25 pounds with a one-time pet fee and monthly pet rent.', 'lifestyle', $1, 2),
        ('What levels of care do you provide?', 'We offer assisted living services including medication management, personal care assistance, and 24-hour on-site staff.', 'care', $2, 1),
        ('How do you handle emergencies?', 'All residents have emergency call systems, and our trained staff are available 24/7 to respond to any situation.', 'care', $2, 2),
        ('What makes your memory care program special?', 'Our memory care program uses evidence-based approaches including cognitive stimulation, structured routines, and personalized care plans.', 'care', $3, 1),
        ('Can family members visit anytime?', 'Family visits are encouraged and welcome during our open visiting hours from 8 AM to 8 PM daily.', 'lifestyle', $3, 2)
    ...
    const galleries = await sql`
      ...
      VALUES 
        ('Sunset Manor Gallery', 'Beautiful spaces and amenities at Sunset Manor', $1, '[
        ...
        ('Garden Oaks Spaces', 'Peaceful gardens and caring environments', $2, '[
        ...
        ('Heritage Hills Life', 'Dignified living and specialized care', $3, '[
