...
// Script to delete all FAQs and create new generalized ones
...
// New generalized FAQs for Stage Senior
const stageSeniorFAQs: FAQ[] = [
  {
...
// Function to get all existing FAQs
async function getAllFAQs(): Promise<any[]> {
  try {
...
async function main() {
  ...
  try {
    ...
    if (existingFAQs.length > 0) {
      console.log("🗑️  Deleting existing FAQs...");
      for (const faq of existingFAQs) {
        await deleteFAQ(faq.id);
        // Small delay to avoid overwhelming the server
        await new Promise(resolve => setTimeout(resolve, 50));
      }
      console.log(`\n✅ Deleted ${existingFAQs.length} FAQs\n`);
    }

    // Step 2: Create new Stage Senior FAQs
    ...
    console.log(`\nTotal FAQs created: ${stageSeniorFAQs.length + healthyAtHomeFAQs.length}`);
